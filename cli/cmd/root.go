...
func init() {
	...
	rootCmd.PersistentFlags().BoolVar(&globalConf.NoCtrlChars, "no-ctrl-chars", false, "filter control characters from text output, replacing them with '.'")
...
func setModConfig(globalConf config.BaseConfig, modConf config.IConfig) {
	...
	modConf.SetNoCtrlChars(globalConf.NoCtrlChars)
