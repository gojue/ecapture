...
func init() {
	_ = factory.RegisterProbe(factory.ProbeTypeZsh, func() (domain.Probe, error) {
		return &stubProbe{name: string(factory.ProbeTypeZsh)}, nil
	})
}
...
func (p *stubProbe) Initialize(_ context.Context, _ domain.Configuration) error {
	return errors.NewProbeStartError(p.name, errors.New(errors.ErrCodeConfiguration, "zsh probe is not supported on Windows"))
}
