...
echo "To run comprehensive e2e tests, use:"
echo ""
echo "All tests:"
echo "  sudo make e2e           # Run all e2e tests (bash, zsh, mysql, postgres, tls, gnutls, gotls)"
echo ""
echo "Simple probe tests:"
echo "  sudo make e2e-bash      # Run Bash command capture test"
echo "  sudo make e2e-zsh       # Run Zsh command capture test (requires zsh installed)"
echo "  sudo make e2e-mysql     # Run MySQL query capture test (requires MySQL/MariaDB running)"
echo "  sudo make e2e-postgres  # Run PostgreSQL query capture test (requires PostgreSQL running)"
echo ""
echo "TLS probe tests:"
echo "  sudo make e2e-tls       # Run OpenSSL/BoringSSL test"
echo "  sudo make e2e-gnutls    # Run GnuTLS test"
echo "  sudo make e2e-gotls     # Run GoTLS test"
echo ""
echo "These tests require root privileges and will capture real traffic."
