...
func TestHkdf(t *testing.T) {
	...
	// TODO test with different cipher_id
	transcript := crypto.SHA256.New()
	transcript.Write(handshakeTrafficHash)

	clientSecret := ExpandLabel(handshakeSecret,
		ClientHandshakeTrafficLabel, transcript.Sum(nil), transcript.Size())
	t.Logf("%s: %x", KeyLogLabelClientHandshake, clientSecret)

	serverHandshakeSecret := ExpandLabel(handshakeSecret,
		ServerHandshakeTrafficLabel, transcript.Sum(nil), transcript.Size())
	t.Logf("%s: %x", KeyLogLabelServerHandshake, serverHandshakeSecret)

	transcript.Reset()
	transcript.Write(serverFinishedHash)

	trafficSecret := ExpandLabel(masterSecret,
		ClientApplicationTrafficLabel, transcript.Sum(nil), transcript.Size())
	t.Logf("%s: %x", KeyLogLabelClientTraffic, trafficSecret)

	serverSecret := ExpandLabel(masterSecret,
		ServerApplicationTrafficLabel, transcript.Sum(nil), transcript.Size())
