...
func (h *exportHandler) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	if req.Method != "GET" {
		...
		log.Printf("har.ServeHTTP: method not allowed: %s", req.Method)
		return
	}
	log.Printf("exportHandler.ServeHTTP: writing HAR logs to ResponseWriter")
...
func (h *resetHandler) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	if !(req.Method == "POST" || req.Method == "DELETE") {
		...
		rw.WriteHeader(http.StatusMethodNotAllowed)
		log.Printf("har: method not allowed: %s", req.Method)
		return
	}

	v, err := parseBoolQueryParam(req.URL.Query(), "return")
	if err != nil {
		log.Printf("har: invalid value for return param: %s", err)
	...
	log.Printf("resetHandler.ServeHTTP: HAR logs cleared")
}
