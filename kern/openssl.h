...
static __inline int kretprobe_connect(struct pt_regs *ctx, int fd, struct sock *sk) {
    u64 current_pid_tgid = bpf_get_current_pid_tgid();
    u32 pid = current_pid_tgid >> 32;
    u64 current_uid_gid = bpf_get_current_uid_gid();
    u32 uid = current_uid_gid;
...
int retprobe_connect(struct pt_regs* ctx) {
    ...
    if (fd_info) {
        ...
        if (sk) {
            return kretprobe_connect(ctx, fd_info->fd, sk);
        }
    }
    return 0;
}
...
int retprobe_accept4(struct pt_regs* ctx) {
    ...
    if (fd_info) {
        sk = tcp_sock_from_file(fd_info->file);
        if (sk) {
            return kretprobe_connect(ctx, fd, sk);
        }
    }
    return 0;
}
